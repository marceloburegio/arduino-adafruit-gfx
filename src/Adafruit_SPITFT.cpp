...
// beteween the SPIImpl pointer (argument #3) and a regular integer.
...
// but if there's any library out there that creates a 'virtual' SPIImpl
...
/*!
    ...
    @param   spiClass  Pointer to SPIImpl type (e.g. &SPI or &SPI1).
    @param   cs        Arduino pin # for chip-select (-1 if unused, tie CS low).
    @param   dc        Arduino pin # for data/command select (required).
    @param   rst       Arduino pin # for display reset (optional, display reset
                       can be tied to MCU reset, default of -1 means unused).
    @note    Output pins are not initialized in constructor; application
             ...
             (calling the SPIImpl constructor) rather than one of the
...
*/
Adafruit_SPITFT::Adafruit_SPITFT(uint16_t w, uint16_t h, SPIImpl *spiClass,
      ...
      if ((pixelBuf[0] = (uint16_t *)malloc(maxFillLen * sizeof(uint16_t)))) {
        ...
        if ((descriptor = (DmacDescriptor *)memalign(
          ...
          if (connection == TFT_HARD_SPI) {
            ...
            // the SPIImpl pointer (e.g. &SPI or &SPI1), which
            // is all we have to work with. SPIImpl does contain
            // a SERCOM pointer but it is a PRIVATE member!
            // Doing an UNSPEAKABLY HORRIBLE THING here, directly
            // accessing the first 32-bit value in the SPIImpl
...
/*
// @TODO - The writePixels method hasn't been implemented in SPI.h (mongoose / arduino-spi) yet. Disabling the code below.
