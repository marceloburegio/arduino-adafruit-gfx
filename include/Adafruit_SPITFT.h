...
typedef class HardwareSPI SPIImpl;        ///< SPI is a bit odd on WICED
...
public:
  ...
  // width & height (rotation 0), SPIImpl pointer, 2 signal pins (cs, dc)
  // and optional reset pin. cs is required but can be -1 if unused.
  Adafruit_SPITFT(uint16_t w, uint16_t h, SPIImpl *spiClass, int8_t cs,
...
protected:
  ...
  union {
    ...
    struct {          //   Values specific to HARDWARE SPI:
      SPIImpl *_spi; ///< SPI class pointer
